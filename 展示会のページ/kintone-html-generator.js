(function (root) {
  ...
  var HERO_IMG = 'https://commoncom.jp/wp/wp-content/uploads/2026/01/4011255_s.jpg';
  var PHOTO_EXHIBIT = 'https://commoncom.jp/wp/wp-content/uploads/2025/10/d53e5269c9322e4dfba1d426b77ecbb5-300x200.jpg.webp';
  var PHOTO_SEMINAR = 'https://commoncom.jp/wp/wp-content/uploads/2025/10/IMG_0146-1024x768.jpg.webp';
  ...
  function normalizeRecord(record) {
    return {
      ...
      seminars: tableTitles(record, 'seminars', 'seminar_sort', 'seminar_title'),
      exhibits: tableTitles(record, 'exhibits', 'exhibit_sort', 'exhibit_title')
    };
  }
  ...
  function classifyEvents(events) {
    ...
    events.forEach(function (event) {
      ...
      if (!current || event.date > current) latestByPref[event.prefecture] = event.date;
    });
    var upcoming = [];
    var past = [];
    events.forEach(function (event) {
  ...
  function groupHtml(label, titles, listClass, labelClass) {
    ...
    return (
      '<div>' +
        '<p class="' + labelClass + '">＜' + esc(label) + '＞</p>' +
        '<ul class="' + listClass + '">' + listHtml(titles) + '</ul>' +
      '</div>'
    );
  }

  function ctaEvents(upcoming) {
    return upcoming.filter(function (event) { return event.applyUrl; });
  }
  ...
  function renderHero(upcoming, past) {
    ...
    var sub = prefs.length ? prefs.join(' &nbsp;/&nbsp; ') + ' &nbsp;' + year : String(year);
    var ctaHtml = ctas.map(function (event, index) {
      var klass = index === 0 ? 'ccx-btn ccx-btn-o' : 'ccx-btn ccx-btn-g';
      return (
        '<a class="' + klass + '" href="' + esc(event.applyUrl) + '">' +
          '<b>' + esc(mdDate(event.date)) + '</b>' +
          esc(event.prefecture) + 'のお申込みはこちら' +
        '</a>'
      );
    }).join('\n        ');
    return [
  ...
  function renderCardFoot(event, navyClass) {
    var ghosts = [];
    if (event.detailUrl) ghosts.push('<a class="ccx-ghost" href="' + esc(event.detailUrl) + '">詳細はこちら</a>');
    if (event.flyerUrl) ghosts.push('<a class="ccx-ghost" href="' + esc(event.flyerUrl) + '">チラシを見る</a>');
    var apply = event.applyUrl
      ? '<a class="ccx-apply' + navyClass + '" href="' + esc(event.applyUrl) + '">' + esc(event.prefecture) + 'のお申込みは<br>こちら →</a>'
      : '';
    var sub = ghosts.length ? '<div class="ccx-foot-sub">' + ghosts.join('\n            ') + '</div>' : '';
    if (!apply && !sub) return '';
    var cols = apply && sub ? '' : ' style="grid-template-columns:1fr"';
    return (
      '        <div class="ccx-card-foot"' + cols + '>\n' +
      (apply ? '          ' + apply + '\n' : '') +
      (sub ? '          ' + sub + '\n' : '') +
      '        </div>'
    );
  ...
  function renderUpcomingCard(event) {
    ...
    return [
      ...
      '          <div>',
      '            <p class="ccx-date">' + upcomingDateHtml(event.date) + '</p>',
      '            <p class="ccx-place">' + esc(event.prefecture) + ' ／ ' + esc(event.venue) + '</p>',
      '          </div>',
      '          <span class="ccx-tag' + navyClass + '">' + esc(event.eventType) + '</span>',
      '        </div>',
  ...
  function renderUpcoming(upcoming) {
    ...
    var countLabel = upcoming.length + ' EVENT' + (upcoming.length === 1 ? '' : 'S');
    return [
      '',
  ...
  function renderPastEvent(event) {
    ...
    if (event.visitorCompanies !== '' && event.visitors !== '') {
      note = (
        '\n        <p class="ccx-row-note">' + esc(event.visitorCompanies) + '社' + esc(event.visitors) +
        '名様<br>ご来場いただきました！<br>ありがとうございました！</p>'
      );
    }
    var links = [];
    if (event.detailUrl) links.push('<a href="' + esc(event.detailUrl) + '">詳細はこちら</a>');
    if (event.flyerUrl) links.push('<a href="' + esc(event.flyerUrl) + '">チラシを見る</a>');
    return [
      '    <div class="ccx-row">',
      '      <div>',
      '        <p class="ccx-row-date">' + slashDate(event.date) + '</p>',
      '        <p class="ccx-row-place">' + esc(event.prefecture) + ' ／ ' + esc(event.venue) + '</p>' + note,
      '      </div>',
  ...
  function renderPast(past, hasSticky) {
    ...
    return [
      '',
      '  <section class="ccx-sec"' + (hasSticky ? ' style="padding-bottom:90px"' : '') + '>',
  ...
  function renderSticky(upcoming) {
    ...
    var summary = ctas.map(function (event) {
      return event.prefecture + ' ' + mdDate(event.date);
    }).join('・') + ' 開催';
    var buttons = ctas.map(function (event, index) {
      var klass = index === 0 ? 'ccx-btn-o' : 'ccx-btn-g';
      var extra = index === 0 ? ' style="box-shadow:none"' : '';
      return '<a class="' + klass + '" href="' + esc(event.applyUrl) + '"' + extra + '>' + esc(event.prefecture) + 'のお申込み</a>';
    }).join('\n        ');
    return [
      '',
  ...
  function previewDocument(pasteHtml) {
    ...
    return (
      ...
      '<body style="margin:0;background:#fff">' + inner + '</body></html>'
    );
  }
  ...
  function ensurePreviewUi() {
    ...
    root.querySelector('#ccx-preview-close').addEventListener('click', function () {
      root.style.display = 'none';
    });
    root.querySelector('#ccx-preview-tab-preview').addEventListener('click', function () {
    ...
    root.querySelector('#ccx-preview-tab-source').addEventListener('click', function () {
      root.querySelector('#ccx-preview-tab-source').classList.add('is-active');
      root.querySelector('#ccx-preview-tab-preview').classList.remove('is-active');
      frame.style.display = 'none';
      source.style.display = 'block';
    });
    root.querySelector('#ccx-preview-copy').addEventListener('click', function () {
      copyHtml(source.value).then(function () {
  ...
  function fileToObjectUrl(fileKey) {
    return fetch(kintone.api.url('/k/v1/file', true) + '?fileKey=' + encodeURIComponent(fileKey), {
      method: 'GET',
  ...
  function loadHelpImages(root) {
    ...
    kintone.api(kintone.api.url('/k/v1/records', true), 'GET', {
      ...
      var jobs = [];
      if (files[0] && img1) {
        jobs.push(fileToObjectUrl(files[0].fileKey).then(function (url) {
      ...
      if (files[1] && img2) {
        jobs.push(fileToObjectUrl(files[1].fileKey).then(function (url) {
  ...
  function ensureHelpUi() {
    ...
    root.innerHTML =
      ...
      '<div id="ccx-help-body"><article>' +
        ...
        '<ol>' +
          '<li>開催日・都道府県・会場・種別・セミナー／出展・各URLを登録します。</li>' +
        ...
        '<ol>' +
          '<li>「WordPress」ボタンを押すか、' +
            '<a href="' + WP_EDIT_URL + '" target="_blank" rel="noopener noreferrer">固定ページ「展示会」の編集画面</a>' +
            'を開きます。</li>' +
      ...
      '</article></div>';
    document.body.appendChild(root);
    root.querySelector('#ccx-help-close').addEventListener('click', function () {
      root.style.display = 'none';
    });
    return root;
  }
  ...
  function getEvents() {
    ...
    function fetchPage(offset) {
      return kintone.api(kintone.api.url('/k/v1/records', true), 'GET', {
        app: kintone.app.getId(),
        query: 'publish_status in ("公開") order by event_date desc limit 500 offset ' + offset
      }).then(function (resp) {
  ...
  var WP_EDIT_URL = 'https://commoncom.jp/wp/wp-admin/post.php?post=6042&action=edit';
  ...
  function onGenerate() {
    ...
    getEvents().then(function (records) {
  ...
  function createBar(variant, event) {
    ...
    bar.innerHTML =
      ...
      '<a id="ccx-html-wp-link" href="' + WP_EDIT_URL + '" target="_blank" rel="noopener noreferrer">WordPress</a>' +
      '<button type="button" id="ccx-html-help-btn">ヘルプ</button>';
    bar.querySelector('#ccx-html-gen-btn').addEventListener('click', onGenerate);
  ...
  function findDetailPager() {
    return document.querySelector('.gaia-argoui-app-show-toolbar .gaia-argoui-app-pager') ||
      document.querySelector('.gaia-argoui-app-toolbar-menu .gaia-argoui-app-pager') ||
  ...
  function placeDetailButton(attempt) {
    ...
    if (!pager || !pager.parentNode) {
      if (attempt < 25) {
        setTimeout(function () { placeDetailButton(attempt + 1); }, 80);
      }
      return;
    }
